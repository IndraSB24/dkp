                ...
                <table class="table table-bordered table-light shadow-sm p-3 mb-5 bg-white rounded dt-responsive nowrap" id="tabel-produk" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Tanggal Bayar (Estimasi)</th>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="text-center"><?= date('d-m-Y', strtotime($row->tanggal_faktur)); ?></td>
                                <td class="text-center"><?= date('d-m-Y', strtotime($row->tanggal_bayar)); ?></td>
                                <td class="text-center">Rp. <?= number_format($row->total_pembelian, 2, ',', '.'); ?></td>
                                <td class="text-center">Rp. <?= number_format($row->dibayar, 2, ',', '.'); ?></td>
                                <td class="text-center">Rp. <?= number_format($row->total_pembelian - $row->dibayar, 2, ',', '.'); ?></td>
    ...
    <div class="modal-dialog ">
        <div class="modal-content">
            <div class="modal-header bg-primary" style="color:white">
                <h5 class="modal-title" id="staticBackdropLabel">Tambah Data Pembayaran Hutang</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="<?= base_url(); ?>daftar_hutang/bayar/hutang/pembelian" method="POST">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="est_tgl_bayar">Estimasi Tanggal Pembayaran</label>
                        <input type="text" class="form-control" id="est_tgl_bayar" name="est_tgl_bayar" value="<?= date('d-m-Y', strtotime($row->tanggal_bayar)); ?>" readonly />
                    </div>
                    <div class="form-group">
                        <label for="sisa">Sisa Hutang</label>
                        <input type="text" class="form-control" id="sisa" name="sisa" value="Rp. <?= number_format($row->total_pembelian - $row->dibayar, 2, ',', '.'); ?>" readonly />
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="bukti" name="bukti" autocomplete="off" placeholder="MASUKKAN LINK BUKTI BAYAR . . ." />
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="id_vendor" id="id_vendor" value="<?= $row->id_supplier ?>" />
